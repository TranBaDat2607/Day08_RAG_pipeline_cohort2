"""
Task 7 — Reranking Module (group project).

PHƯƠNG PHÁP ĐÃ CHỌN: **MMR (Maximal Marginal Relevance)** — tự implement, KHÔNG
dùng cross-encoder model. Bản merge: tái dùng đúng embedding model E5 đã cache ở
Task 5 (core), nhất quán không gian vector với toàn pipeline FAISS của nhóm.

Vì sao MMR (so với RRF) cho bài này:
    - Contract của Task 7 là rerank(query, candidates, top_k): re-order MỘT danh
      sách ứng viên duy nhất. MMR đúng bản chất việc này — vừa giữ độ liên quan với
      query, vừa loại bớt các chunk gần-trùng-lặp để tăng diversity.
    ...
    - Dữ liệu tiếng Việt ở đây (văn bản luật + báo) có rất nhiều đoạn lặp/boilerplate.
      MMR cắt trùng lặp → context đưa vào LLM đa dạng và bao phủ tốt hơn (Task 10).
...
from ..ingestion.chunk_and_index import normalize_text
from ..ingestion.config import EMBED_PASSAGE_PREFIX, EMBED_QUERY_PREFIX
...
def _get_model():
    ...
    from .task5_semantic_search import _get_model as _task5_model
...
def rerank_mmr(
    ...
    Args:
        ...
        candidates: List of {'content','score','metadata','embedding'?}. Thiếu
                    'embedding' sẽ tự embed từ 'content'.
        top_k: Số lượng kết quả.
        lambda_param: Trade-off relevance (1.0) ↔ diversity (0.0).

    Returns:
        List of ≤ top_k candidates theo đúng thứ tự MMR đã chọn. Mỗi item gắn
        'score' (= relevance với query) và 'mmr_rank' (thứ hạng do MMR quyết định).
    """
    if not candidates:
        return []
