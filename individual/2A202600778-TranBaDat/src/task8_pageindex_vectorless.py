...
Lưu ý: API truy hồi (submit_query/get_retrieval) đã được PageIndex đánh dấu
"deprecated" và khuyến khích chuyển sang chat-completions API. Tuy nhiên nó vẫn
hoạt động và trả về *nguyên văn các đoạn trong tài liệu* (không bịa, không bổ
sung web) — đúng thứ ta cần cho một retriever. Vì vậy ta dùng nó làm đường
chính; nếu nó trả rỗng thì fallback sang chat-completions (có enable_citations)
để rút trích các đoạn được trích dẫn.
"""
