...
MIN_CONTENT_SIZE = 500  # bytes — khớp pytest TestTask2
...
def _crawl_with_requests(url: str) -> dict:
    """Fallback crawl bằng requests + BeautifulSoup."""
...
async def _crawl_with_crawl4ai(url: str) -> dict:
    """Crawl bằng Crawl4AI (cần `playwright install`)."""
...
async def crawl_article(url: str) -> dict:
    ...
    Ưu tiên requests (ổn định); thử Crawl4AI nếu requests thất bại.
    """
    try:
    ...
    for i, url in enumerate(ARTICLE_URLS, 1):
        ...
        if filepath.exists() and not force:
            ...
            if size >= MIN_CONTENT_SIZE:
                print(f"[{i}/{len(ARTICLE_URLS)}] ↷ Đã có: {filename} ({size:,} bytes)")
                continue
...
if __name__ == "__main__":
    ...
    if not ARTICLE_URLS:
        ...
        print("=" * 60)
        print("Task 2: Crawl bài báo")
        print("=" * 60)
        try:
            asyncio.run(crawl_all())
            print(f"\n✓ Hoàn tất — files trong {DATA_DIR}")
